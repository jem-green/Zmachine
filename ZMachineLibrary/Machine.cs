...
namespace ZMachineLibrary
{
	public class Machine
	{
		...
		private const int VersionPointer = 0x00;
		private const int FlagsOnePointer = 0x01;
		private const int BaseHighMemory = 0x04;
		...
		private void SetupOpcodes()
		{
            ...
            {
                _0Opcodes[0x09] = new Opcode { Handler = Catch,         Name = "CATCH" };
            }
            ...
            _2Opcodes[0x1c] = new Opcode { Handler = Throw,           Name = "THROW" };
			...
			_varOpcodes[0x1b] = new Opcode { Handler = Tokenize,      Name = "TOKENISE" };
			_varOpcodes[0x1c] = new Opcode { Handler = EncodeText,    Name = "ENCODE_TEXT" };
			...
			_extOpcodes[0x05] = new Opcode { Handler = DrawPicture,   Name = "DRAW_PICTURE" };
			_extOpcodes[0x06] = new Opcode { Handler = PictureData,   Name = "PICTURE_DATA" };
			_extOpcodes[0x07] = new Opcode { Handler = ErasePicture,  Name = "ERASE_PICTURE" };
			_extOpcodes[0x08] = new Opcode { Handler = SetMargins,    Name = "SET_MARGINS" };
			_extOpcodes[0x09] = new Opcode { Handler = SaveUndo,      Name = "SAVE_UNDO" };
            _extOpcodes[0x0a] = new Opcode { Handler = RestoreUndo,   Name = "RESTORE_UNDO" };
            _extOpcodes[0x0b] = new Opcode { Handler = PrintUnicode,  Name = "PRINT_UNICODE" };
            _extOpcodes[0x0c] = new Opcode { Handler = CheckUnicode,  Name = "CHECK_UNICODE" };

			if (_version >= 5)
			{
				_extOpcodes[0x0d] = new Opcode { Handler = SetTrueColor, Name = "SET_TRUE_COLOR" };
			}
			...
			_extOpcodes[0x10] = new Opcode { Handler = MoveWindow, Name = "MOVE_WINDOW" };
			_extOpcodes[0x11] = new Opcode { Handler = WindowSize, Name = "WINDOW_SIZE" };
			_extOpcodes[0x12] = new Opcode { Handler = WindowStyle, Name = "WINDOW_SIZE" };
			_extOpcodes[0x13] = new Opcode { Handler = GetWindowProp, Name = "WINDOW_SIZE" };
			_extOpcodes[0x14] = new Opcode { Handler = ScrollWindow, Name = "WINDOW_SIZE" };
			_extOpcodes[0x15] = new Opcode { Handler = PopStack, Name = "WINDOW_SIZE" };
			_extOpcodes[0x16] = new Opcode { Handler = ReadMouse, Name = "WINDOW_SIZE" };
			_extOpcodes[0x17] = new Opcode { Handler = MouseWindow, Name = "WINDOW_SIZE" };
			_extOpcodes[0x18] = new Opcode { Handler = PushStack, Name = "WINDOW_SIZE" };
			_extOpcodes[0x19] = new Opcode { Handler = PutWindProp, Name = "WINDOW_SIZE" };
			_extOpcodes[0x1a] = new Opcode { Handler = PrintForm, Name = "WINDOW_SIZE" };
			_extOpcodes[0x1b] = new Opcode { Handler = MakeMenu, Name = "WINDOW_SIZE" };
			_extOpcodes[0x1c] = new Opcode { Handler = PictureTable, Name = "WINDOW_SIZE" };
		}

        private void EncodeText(List<ushort> args)
        {
            throw new NotImplementedException();
        }

        private void Tokenize(List<ushort> args)
        {
			/*
			 ...
			 *
			 */
			TraceInternal.TraceVerbose("Tokenize");
			//throw new NotImplementedException();
        }

        private void WindowStyle(List<ushort> args)
        {
            throw new NotImplementedException();
        }

        private void GetWindowProp(List<ushort> args)
        {
            throw new NotImplementedException();
        }

        private void ScrollWindow(List<ushort> args)
        {
            throw new NotImplementedException();
        }

        private void PopStack(List<ushort> args)
        {
            throw new NotImplementedException();
        }

        private void ReadMouse(List<ushort> args)
        {
            throw new NotImplementedException();
        }

        private void MouseWindow(List<ushort> args)
        {
            throw new NotImplementedException();
        }

        private void PushStack(List<ushort> args)
        {
            throw new NotImplementedException();
        }

        private void PutWindProp(List<ushort> args)
        {
            throw new NotImplementedException();
        }

        private void PrintForm(List<ushort> args)
        {
            throw new NotImplementedException();
        }

        private void MakeMenu(List<ushort> args)
        {
            throw new NotImplementedException();
        }

        private void PictureTable(List<ushort> args)
        {
            throw new NotImplementedException();
        }

        private void WindowSize(List<ushort> args)
        {
            throw new NotImplementedException();
        }

        private void MoveWindow(List<ushort> args)
        {
            throw new NotImplementedException();
        }

        private void SetMargins(List<ushort> args)
        {
            throw new NotImplementedException();
        }

        private void ErasePicture(List<ushort> args)
        {
            throw new NotImplementedException();
        }

        private void PictureData(List<ushort> args)
        {
            throw new NotImplementedException();
        }
        ...
        public void LoadFile(Stream stream)
		{
			...
			_version            = _memory[VersionPointer];
			if (_version >= 3)
            {
				_flagsOne = _memory[FlagsOnePointer];
            }
			...
			_flagsTwo           = GetByte(FlagsTwoPointer);

			// TODO: set these via IZMachineIO

			_memory[0x01] = 0x01;	// Set the status to be hours and minutes
			_memory[0x20] = 25;		// Set the screen height to 25
			_memory[0x21] = 80;		// Set the screen width to 80
			...
			StackFrame zsf = new StackFrame { PC = _pc };
			_stack.Push(zsf);
		}
		...
		public void Run(bool terminateOnInput = false)
		{
			...
			while(_running)
			{
				...
				TraceInternal.TraceVerbose($"PC: {_stack.Peek().PC:X5}");
				byte o = _memory[_stack.Peek().PC++];
                if (o == 0xbe)  // = 190
                {
                    o = _memory[_stack.Peek().PC++];
                    opcode = _extOpcodes?[o & 0x1f];    // 31
		...
		public Stream SaveState()
		{
			...
			DataContractJsonSerializer dcs = new DataContractJsonSerializer(typeof(Stack<StackFrame>));
		...
		public void RestoreState(Stream stream)
		{
			...
			DataContractJsonSerializer dcs = new DataContractJsonSerializer(typeof(Stack<StackFrame>));
			_stack = (Stack<StackFrame>)dcs.ReadObject(stream);
			stream.Dispose();
		}
		...
		private void PrintTable(List<ushort> args)
		{
			...
			_io.Print(s);
			TraceInternal.TraceInformation($"[{s}]");
		}
		...
		private void PrintRet(List<ushort> args)
		{
			List<byte> chars = GetZsciiChars(_stack.Peek().PC);
			_stack.Peek().PC += (ushort)(chars.Count/3*2);
			string s = DecodeZsciiChars(chars) + Environment.NewLine;
			_io.Print(s);
			TraceInternal.TraceInformation($"[{s}]");
			RTrue(null);
		}
		...
		private void SetFont(List<ushort> args)
		{
			...
			byte dest = _memory[_stack.Peek().PC++];
			StoreWordInVariable(dest, 0);
			throw new Exception("SetFont");
		}

		private void DrawPicture(List<ushort> args)
        {
			// TODO - the rest of the params
			throw new Exception("DrawPicture");
		}

		private void SaveUndo(List<ushort> args)
        {
			// TODO - the rest of the params
			Debug.WriteLine("SaveUndo called ?");
			//throw new Exception("SaveUndo");
        }

        private void RestoreUndo(List<ushort> args)
        {
            // TODO - the rest of the params
            throw new Exception("RestoreUndo");
        }

        private void PrintUnicode(List<ushort> args)
        {
            // TODO - the rest of the params
            throw new Exception("PrintUnicode");
        }

        private void CheckUnicode(List<ushort> args)
        {
            // TODO - the rest of the params
            throw new Exception("CheckUnicode");
        }

        private void SetTrueColor(List<ushort> args)
        {
            // TODO - the rest of the params
            throw new Exception("SetTrueColor");
        }

        private void Throw(List<ushort> args)
		{
            // TODO - the rest of the params
		...
		public void FinishRead(string input)
		{
			if(input != null && _readTextAddr != 0 && _readParseAddr != 0)
			{
				...
				TraceInternal.TraceVerbose($"[{input}]");
		...
		private void InsertObj(List<ushort> args)
		{
			...
			TraceInternal.TraceVerbose($"[{GetObjectName(args[0])}] [{GetObjectName(args[1])}] ");
		...
		private void RemoveObj(List<ushort> args)
		{
			...
			TraceInternal.TraceVerbose($"[{GetObjectName(args[0])}] ");
		...
		private void GetParent(List<ushort> args)
		{
			...
			TraceInternal.TraceVerbose($"[{GetObjectName(parent)}] ");
		...
		private void GetChild(List<ushort> args)
		{
			...
			TraceInternal.TraceVerbose($"[{GetObjectName(child)}] ");
		...
		private void GetSibling(List<ushort> args)
		{
			...
			TraceInternal.TraceVerbose($"[{GetObjectName(sibling)}] ");
		...
		private void Jump(List<ushort> args)
		{
			...
			TraceInternal.TraceVerbose($"-> {_stack.Peek().PC:X5}");
		}
		...
		{
			TraceInternal.TraceVerbose($"C[{GetObjectName(args[0])}] P[{GetObjectName(args[1])}] ");
		...
		private void Jump(bool flag)
		{
			...
			if((offset & 0x80) == 0x80)
			{
				TraceInternal.TraceVerbose(" [TRUE] ");
				branch = true;
			}
			else
			{
				TraceInternal.TraceVerbose(" [FALSE] ");
				branch = false;
			}
			...
			if((offset & 0x40) == 0x40)
			{
				...
				{
					TraceInternal.TraceVerbose(" RFALSE ");
					RFalse(null);
					return;
				}
				...
				{
					TraceInternal.TraceVerbose(" RTRUE ");
					RTrue(null);
					return;
				}
		...
		private void Call(List<ushort> args, bool storeResult)
		{
			...
			TraceInternal.TraceVerbose($"New PC: {pc:X5}");

			StackFrame zsf = new StackFrame { PC = pc, StoreResult = storeResult };
			_stack.Push(zsf);
        ...
        private void Catch(List<ushort> args)
        {
            ushort val = 0;
            byte dest = _memory[_stack.Peek().PC++];
            StoreWordInVariable(dest, (ushort)val);
				...
				case OperandType.LargeConstant:
                    {
                        ...
                        _stack.Peek().PC += 2;
                        TraceInternal.TraceVerbose($"#{arg:X4}, ");
                        break;
                    }
				case OperandType.SmallConstant:
                    {
                        arg = _memory[_stack.Peek().PC++];
                        TraceInternal.TraceVerbose($"#{arg:X2}, ");
                        break;
                    }
		...
		private void StoreByteInVariable(byte dest, byte value)
		{
			if(dest == 0)
			{
				TraceInternal.TraceVerbose($"-> SP ({value:X4}), ");
				_stack.Peek().RoutineStack.Push(value);
			}
			else if(dest < 0x10)
			{
				TraceInternal.TraceVerbose($"-> L{dest-1:X2} ({value:X4}), ");
				_stack.Peek().Variables[dest-1] = value;
			}
			else
			{
				// this still gets written as a word...write the byte to addr+1
				TraceInternal.TraceVerbose($"-> G{dest - 0x10:X2} ({value:X4}), ");
		...
		private ushort GetVariable(byte variable, bool pop=true)
		{
			...
			if(variable == 0)
			{
				...
				TraceInternal.TraceVerbose($"SP ({val:X4}), ");
			}
			else if(variable < 0x10)
			{
				val = _stack.Peek().Variables[variable-1];
				TraceInternal.TraceVerbose($"L{variable-1:X2} ({val:X4}), ");
			}
			else
			{
				val = GetWord((ushort)(_globals + 2 * (variable - 0x10)));
				TraceInternal.TraceVerbose($"G{variable - 0x10:X2} ({val:X4}), ");
			}
			return val;
		}
		...
		private ushort PrintObjectInfo(ushort obj, bool properties)
		{
			...
			TraceInternal.TraceVerbose($"{obj} ({obj:X2}) at {propAddr:X5}: ");
			...
			TraceInternal.TraceVerbose($"[{s}] A:{attributes:X12} P:{parent}({parent:X2}) S:{sibling}({sibling:X2}) C:{child}({child:X2})");
			...
			if(properties)
			{
				...
				TraceInternal.TraceVerbose("Attributes: " + ss);
				...
				while(_memory[propAddr] != 0x00)
				{
					...
					TraceInternal.TraceVerbose($"  P:{propNum:X2} at {propAddr:X4}: ");
                    for (int i = 0; i < len; i++)
                    {
                        TraceInternal.TraceVerbose($"{_memory[propAddr++]:X2} ");
                    }
					TraceInternal.TraceVerbose("");
					propAddr++;
				}
		...
		private void PrintDictionary()
		{
			...
			TraceInternal.TraceVerbose("Separators: [" );
            for (int i = 0; i < len; i++)
            {
                TraceInternal.TraceVerbose(Convert.ToChar(_memory[address++]).ToString());
            }
			TraceInternal.TraceVerbose("]");
			...
			ushort numEntries = GetWord(address);	
			address+=2;
			TraceInternal.TraceVerbose($"Entry Length: {entryLength}, Num Entries: {numEntries}");
